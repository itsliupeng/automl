...
class GpuBatchNormalization(tf.layers.BatchNormalization):
    def __init__(self, fused=False, **kwargs):
        if fused in (True, None):
            raise ValueError('GpuBatchNormalization does not support fused=True.')
        super(GpuBatchNormalization, self).__init__(fused=fused, **kwargs)
    ...
    def _moments(self, inputs, reduction_axes, keep_dims):
        """Compute the mean and variance: it overrides the original _moments."""
        shard_mean, shard_variance = super(GpuBatchNormalization, self)._moments(
            inputs, reduction_axes, keep_dims=keep_dims)
        ...
        if num_shards <= 8:  # Skip cross_replica for 2x2 or smaller slices.
            ...
            num_shards_per_group = max(8, num_shards // 8)
        tf.logging.info('GpuBatchNormalization with num_shards_per_group {}'.format(
            num_shards_per_group))
        if num_shards_per_group > 1:
            # Compute variance using: Var[X]= E[X^2] - E[X]^2.
            shard_square_of_mean = tf.math.square(shard_mean)
