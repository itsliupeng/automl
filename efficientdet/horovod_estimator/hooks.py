...
class TrainImageVisualizationHook(tf.train.SessionRunHook):
    def __init__(self, images_name, labels_name, filenames_name, probs_name, raw_images_name=None,
                 heat_map_features_name=None, every_n_steps=100, summary_dir=None, max_images=8):
        assert images_name is not None and labels_name is not None and probs_name is not None
        self._images_name = images_name
        self._labels_name = labels_name
        self._filenames_name = filenames_name
        self._raw_images_name = raw_images_name
                ...
                for name in [self._images_name, self._labels_name, self._filenames_name, self._raw_images_name,
                    ...
                    if name is not None:
                        try:
                            ...
                            tf.logging.debug('{} warn {}'.format(name, e))

                arg_map['global_step'] = tf.train.get_or_create_global_step()
                self._arg_map = arg_map

            return SessionRunArgs(self._arg_map)
    ...
    def after_run(self, run_context, run_values):
        if self._step % self._every_n_steps == 0:
            ...
            heat_map_features = get_key_or_none(results, self._heat_map_features_name)
            probs = get_key_or_none(results, self._probs_name)
            global_step = get_key_or_none(results, 'global_step')
...
class ConfusionMatrixHook(basic_session_run_hooks.SecondOrStepTimer, tf.train.SessionRunHook):
    def __init__(self, num_classes, labels_name, predicts_name, summary_dir):
    ...
    def before_run(self, run_context):
        return SessionRunArgs({'labels': basic_session_run_hooks._as_graph_element(self._labels_name),
    ...
    def end(self, session):
        try:
            ...
            summary = confusion_matrix_summary('cm', cnf_matrix, classes, normalize=False)
            summary_recall = confusion_matrix_summary('cm_norm_recall', cnf_matrix, classes, normalize=True,
                                                      recall=True)
            summary_precision = confusion_matrix_summary('cm_norm_precision', cnf_matrix, classes, normalize=True,
                                                         recall=False)
            summary_writer.add_summary(summary, self._global_step)
            summary_writer.add_summary(summary_recall, self._global_step)
            summary_writer.add_summary(summary_precision, self._global_step)

            # roc figure
            # bug:     fpr[i], tpr[i], _ = roc_curve(y_true[:, i], y_pred[:, i])
            #           IndexError: index 1 is out of bounds for axis 1 with size 1
            # summary_roc = roc_summary('roc', all_labels_np, all_predicts_np, self._num_classes)
            # summary_writer.add_summary(summary_roc, self._global_step)

            from sklearn.metrics import classification_report
            report = classification_report(all_labels_np, all_predicts_np, digits=4)
            # to-do: save it to file
            with open(os.path.join(self._summary_dir, 'classification_report.txt'), 'w') as f:
                f.write(report)

            print(report)
        except Exception as e:
            print('Exception in ConfusionMatrixHook: {}'.format(e))


class CropBoundingBoxHook(tf.train.SessionRunHook):
    def __init__(self, images_name, labels_name, filenames_name, heat_map_features_name, probs_name, bbox_filename):
        self._images_name = images_name
        self._labels_name = labels_name
        self._filenames_name = filenames_name
        self._heat_map_features_name = heat_map_features_name
        self._probs_name = probs_name
        self._bbox_filename = bbox_filename

    def begin(self):
        if hvd is not None:
            self._bbox_filename += '.rank{}'.format(hvd.rank())

        arg_map = {}
        for name in [self._images_name, self._labels_name, self._filenames_name, self._heat_map_features_name, self._probs_name]:
            if name is not None:
                try:
                    arg_map[name] = basic_session_run_hooks._as_graph_element(name)
                except Exception as e:
        ...
        with open(self._bbox_filename, 'w') as f:
            f.write('id,label,label_prob,predict,predict_prob,xmin,ymin,xmax,ymax\n')
    ...
    def after_run(self, run_context, run_values):
        ...
        with open(self._bbox_filename, 'a') as f:
            for i in range(n):
                if heat_map_features is not None:
                    # cam
                    label = labels[i]
                    cam = heat_map_features[i][:, :, label]
                    cam = cam - np.min(cam)
                    cam_img = cam / np.max(cam)
                    cam_img = np.uint8(255 * cam_img)
                    heat_map = cv2.applyColorMap(cv2.resize(cam_img, (h, w)), cv2.COLORMAP_JET)
                    blue_map = heat_map[:, :, -1]
                    xmin, ymin, xmax, ymax = find_xy(blue_map, percentile=True)
                ...
                if probs is not None:
                    ...
                    predict_prob = probs[i][predict]
                    label_prob = probs[i][label]
                else:
                    predict = -1
                    predict_prob = 0.0
                    label_prob = 0.0
                ...
                f.write('{},{},{:.4f},{},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}\n'.format(
                    filename, label, label_prob, predict, predict_prob, xmin, ymin, xmax, ymax))
